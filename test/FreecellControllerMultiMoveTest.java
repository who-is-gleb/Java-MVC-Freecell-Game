...
/**
 * A test class for the FreecellController using multi-move model.
 * Contains all of the tests I came up with for FreecellController but using FreecellModelMulti
 * class.
 * Also, some specific multi-move tests have been added.
 */
public class FreecellControllerMultiMoveTest {

  @Test
  public void testSingleValidMove() {
    FreecellModel model = new FreecellModelMulti();
  ...
  @Test(expected = IllegalArgumentException.class)
  public void testNullForAppendable() {
    FreecellModel model = new FreecellModelMulti();
    StringBuilder gameLog = null;
    StringReader input = new StringReader("C1 13 F1 C2 13 C1 C2 12 O1 C1 12 C2 C2 10 O1 q");
    IFreecellController<Card> controller = new FreecellController(input, gameLog);
    controller.playGame(model.getDeck(), model, 4, 5, false);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testNullForReadable() {
    FreecellModel model = new FreecellModelMulti();
    StringBuilder gameLog = new StringBuilder();
  ...
  // new tests for multi-moving
  @Test
  public void testSingleValidMultiMove() {
    FreecellModel model = new FreecellModelMulti();
    StringBuilder gameLog = new StringBuilder();
    StringReader input = new StringReader("C1 13 F1 C2 13 C1 C2 12 O1 C1 12 C2 q");
    IFreecellController<Card> controller = new FreecellController(input, gameLog);
    controller.playGame(model.getDeck(), model, 4, 5, false);

    System.out.println(model.getGameState());
    assertEquals(11, model.getCascadePiles().get(0).size());
    assertEquals(13, model.getCascadePiles().get(1).size());
  }
  ...
  @Test
  public void testInvalidMultiMoveFollowedByValidMultiMove() {
    FreecellModel model = new FreecellModelMulti();
    StringBuilder gameLog = new StringBuilder();
    StringReader input = new StringReader("C1 13 F1 C2 13 C1 C2 12 O1 C1 11 C2 C1 12 C2 q");
    IFreecellController<Card> controller = new FreecellController(input, gameLog);
    controller.playGame(model.getDeck(), model, 4, 5, false);
    ...
    assertEquals(82, lines.length);
    assertEquals("C4: K♣, Q♣, J♣, 10♣, 9♣, 8♣, 7♣, 6♣, 5♣, 4♣, 3♣, 2♣, A♣\n" +
            "\n" +
            "Please enter your source pile, card index, and dest pile:\n" +
            "Game quit prematurely.", lastMsg);
